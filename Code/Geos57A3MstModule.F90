...
!BOP
!
! !MODULE: Geos57A3MstModule
!
! !DESCRIPTION: Module Geos57A3MstModule contains routines to create the 
...
100 FORMAT ( '%%% Defining netCDF file vars & attrs for ', a' grid' )
    ...
    ! PFICU
    IF ( StrPos( 'PFICU', tavg3_3d_mst_Ne_data_m ) >= 0 ) THEN
       var4  = (/ idLon, idLat, idAp, idTime /)    
       vId   = vId + 1
    ...
    ! PFILSAN
    IF ( StrPos( 'PFILSAN', tavg3_3d_mst_Ne_data_m ) >= 0 ) THEN
       var4  = (/ idLon, idLat, idAp, idTime /)    
       vId   = vId + 1
    ...
    ! PFLCU
    IF ( StrPos( 'PFLCU', tavg3_3d_mst_Ne_data_m ) >= 0 ) THEN
       var4  = (/ idLon, idLat, idAp, idTime /)    
       vId   = vId + 1
    ...
    ! PFLLSAN
    IF ( StrPos( 'PFLLSAN', tavg3_3d_mst_Ne_data_m ) >= 0 ) THEN
       var4  = (/ idLon, idLat, idAp, idTime /)    
       vId   = vId + 1
...
!BOP
!
! !IROUTINE: Geos57MakeA3Mst
!
! !DESCRIPTION: Routine Geos57MakeA3Mst is the the driver routine for 
! \begin{enumerate}
...
!
    ...
    ! Echo info    
    msg = '%%%%%%%%%% ENTERING ROUTINE Geos57MakeA3Mst %%%%%%%%%%'
    WRITE( IU_LOG, '(a)' )
    WRITE( IU_LOG, '(a)' ) TRIM( msg )

    ! List of all the A-3 fields combined
    allFieldsList = TRIM( tavg3_3d_mst_Ne_data_m ) // ',' // &
    ...
    CALL GetNFields( tavg3_3d_mst_Ne_data_m, nFields_3dMstNe, fields_3dMstNe )
...
110 FORMAT( '%%% TOTAL # OF FIELDS TO BE REGRIDDED : ', i5 )
    ...
    ! Close binary files
    msg = '%%% Closing A3mst output files'
    WRITE( IU_LOG, '(a)' ) TRIM( msg )
    ...
    ! Echo info
    msg = '%%%%%%%%%% LEAVING ROUTINE Geos57MakeA3mst %%%%%%%%%%'
    WRITE( IU_LOG, '(a)' ) '%%%'   
    WRITE( IU_LOG, '(a)' ) TRIM( msg )

  END SUBROUTINE Geos57MakeA3Mst
!EOC
!------------------------------------------------------------------------------
!          Harvard University Atmospheric Chemistry Modeling Group            !
!------------------------------------------------------------------------------
...
!
    ...
    IF ( do2x25 ) THEN
       ...
       CALL NcGet_DimLen( fOut2x25,   'ap',   Z2x25   ) 
    ...
    IF ( do4x5 ) THEN
       ...
       CALL NcGet_DimLen( fOut4x5,    'ap',   Z4x5    )   
